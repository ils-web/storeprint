  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session) {
        ...
        setErrorMessage('Неверный логин или пароль. Проверьте введённые данные.');
      }
    } catch (err: any) {
      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 pb-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">Вход в StorePrint</h3>
              <p className="text-xs text-slate-400">Мультитенантная платформа</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">
              {activeTab === 'tenant' ? 'Логин филиала / Email' : 'Логин суперадмина'}
            </label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/90 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 font-mono"
              />
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-slate-800/80">
            <p className="text-[11px] text-slate-500 text-center mb-2">Быстрый тестовый вход (Демо):</p>
