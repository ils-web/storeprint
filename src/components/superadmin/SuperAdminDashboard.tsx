...
export function SuperAdminDashboard({
  ...
  const estimatedMonthlyRevenue = tenantsList.reduce((acc, t) => {
    return acc + (BILLING_PLANS[t.plan]?.priceMonthlyUsd || 0);
  }, 0);
  ...
  const handleCreateTenant = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.login.trim() || !formData.password.trim()) {
      alert('Пожалуйста, заполните название, логин и пароль филиала.');
      return;
    }
  ...
  const handleDeleteTenant = (tenant: Tenant) => {
    if (confirm(`Вы уверены, что хотите удалить филиал "${tenant.name}"? Это действие необратимо.`)) {
  ...
  const handleImpersonate = (tenant: Tenant) => {
    saveAuthSession({
      ...
      username: `${tenant.name} (Суперадмин)`,
    });
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Центральная панель управления филиалами и тарифами</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-slate-800/80 p-1 rounded-xl border border-slate-700">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span>Филиалы ({totalTenants})</span>
                </div>
              </button>
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-2 px-3 py-2 text-sm text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors border border-transparent hover:border-rose-500/30"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-slate-800/60 border border-slate-700/70 rounded-2xl p-5 shadow-lg">
            <div className="flex items-center justify-between text-slate-400 mb-2">
              <span className="text-sm font-medium">Всего филиалов</span>
              <Building2 className="w-5 h-5 text-indigo-400" />
            </div>
            <div className="text-3xl font-extrabold text-white">{totalTenants}</div>
            <div className="mt-2 text-xs text-emerald-400 flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5" />
              <span>{activeTenants} активных • {trialTenants} на триале</span>
            </div>
          </div>

          <div className="bg-slate-800/60 border border-slate-700/70 rounded-2xl p-5 shadow-lg">
            <div className="flex items-center justify-between text-slate-400 mb-2">
              <span className="text-sm font-medium">Складов в сети</span>
          ...
          <div className="bg-slate-800/60 border border-slate-700/70 rounded-2xl p-5 shadow-lg">
            <div className="flex items-center justify-between text-slate-400 mb-2">
              <span className="text-sm font-medium">Расчётный MRR</span>
              <CreditCard className="w-5 h-5 text-emerald-400" />
            </div>
            <div className="text-3xl font-extrabold text-white">${estimatedMonthlyRevenue}</div>
            <div className="mt-2 text-xs text-indigo-300">
              Готовность к Stripe / Webhooks
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-900/60 to-purple-900/60 border border-indigo-500/40 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
            <div>
              <span className="text-xs font-semibold uppercase tracking-wider text-indigo-300">Быстрое действие</span>
              <h3 className="text-base font-bold text-white mt-1">Добавить новый филиал</h3>
            </div>
            <button
            ...
            <div className="bg-slate-800/60 border border-slate-700/70 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-900/90 border border-slate-700 rounded-xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <select
                  ...
                  className="px-3 py-2 bg-slate-900/90 border border-slate-700 rounded-xl text-sm text-slate-200 focus:outline-none focus:border-indigo-500"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-400 font-mono mt-0.5">slug: {tenant.slug}</p>
                        </div>
                      ...
                      <div className="bg-slate-900/60 rounded-xl p-3.5 border border-slate-800 space-y-2 mb-4">
                        ...
                        <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-800">
                          ...
                          <span className="font-semibold text-purple-300">
                            {planDetails?.name || tenant.plan} (${planDetails?.priceMonthlyUsd || 0}/мес)
                          </span>
                        </div>
                      </div>

                      {/* Warehouses & Departments Summary */}
                      <div className="space-y-1.5 text-xs text-slate-300 mb-4">
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Складов филиала:</span>
                          <span className="font-medium text-white">{tenantWarehouses.length} склад(а)</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400">Создание складов филиалом:</span>
                          <span className={tenant.limits.allowSelfWarehouseCreation ? 'text-emerald-400 font-semibold' : 'text-slate-400'}>
                            {tenant.limits.allowSelfWarehouseCreation ? 'Разрешено' : 'Централизованно (Админ)'}
                          </span>
                        </div>
                    ...
                    <div className="pt-3 border-t border-slate-700/60 flex items-center justify-between gap-2">
                      <button
                        ...
                        title="Открыть панель склада этого филиала"
                      >
                        <ExternalLink className="w-3.5 h-3.5" />
                        <span>Войти в филиал</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Управление складами филиала"
                      >
                      ...
                      <button
                        ...
                        className="p-2 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-xl transition-colors cursor-pointer"
                        title="Редактировать параметры и пароль"
                      >
              ...
              {filteredTenants.length === 0 && (
                <div className="col-span-full py-12 text-center text-slate-400 bg-slate-800/40 rounded-2xl border border-dashed border-slate-700">
                  ...
                  <p className="text-base font-medium">Филиалы не найдены</p>
                  <p className="text-xs text-slate-500 mt-1">Попробуйте изменить поисковый запрос или создайте новый филиал</p>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-bold text-white">Модуль Подписок и Онлайн Оплаты</h2>
                  </div>
                  <p className="text-sm text-slate-300 mt-1">
                    Архитектура готова к привязке Stripe, PayPal и автоматическому выставлению счетов.
                  </p>
                </div>
                <div className="px-4 py-2 bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 rounded-xl text-xs font-semibold flex items-center gap-2">
                  ...
                  <span>Stripe Webhooks API Hook Ready</span>
                </div>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-700/60 text-center">
                    <span className="text-xs text-slate-400">
                      Лимит складов: <strong className="text-white">{plan.maxWarehouses}</strong>
                    </span>
                  </div>
                </div>
              ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-150">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950">
              <div className="flex items-center gap-2">
                <Building2 className="w-5 h-5 text-indigo-400" />
                <h3 className="font-bold text-lg text-white">Создание нового филиала</h3>
              </div>
              <button
            ...
            <form onSubmit={handleCreateTenant} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Например: Больница Ассута - Корпус 2"
                  value={formData.name}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-indigo-500"
                  >
                    <option value="free">Базовый (Бесплатно)</option>
                    <option value="starter">Стартовый ($29/мес)</option>
                    <option value="pro">Pro ($79/мес)</option>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="г. Тель-Авив, ул. Вайцман 14"
              ...
              {/* Centralized vs Self Warehouse Creation Flag */}
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 flex items-start gap-3">
                ...
                <label htmlFor="selfWhCheck" className="text-xs text-slate-300 cursor-pointer">
                  <strong className="text-white block">Разрешить филиалу создавать склады самостоятельно</strong>
                  Если выключено — склады для филиала создаются централизованно только суперадмином.
                </label>
              </div>
      ...
      {isEditModalOpen && selectedTenant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950">
              <h3 className="font-bold text-lg text-white">Редактирование филиала</h3>
              <button
            ...
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4">
              ...
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 flex items-start gap-3">
                <input
                  type="checkbox"
                  id="selfWhEditCheck"
                  checked={editFormData.allowSelfWarehouseCreation}
                  onChange={(e) => setEditFormData({ ...editFormData, allowSelfWarehouseCreation: e.target.checked })}
                  className="mt-1 w-4 h-4 rounded text-indigo-600"
                />
                <label htmlFor="selfWhEditCheck" className="text-xs text-slate-300 cursor-pointer">
                  <strong className="text-white block">Разрешить филиалу создавать склады</strong>
                  Филиал сможет самостоятельно добавлять подсклады.
                </label>
              </div>
      ...
      {isWarehouseModalOpen && selectedTenant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between bg-slate-950">
              <div>
                <h3 className="font-bold text-lg text-white">Склады филиала</h3>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="space-y-2">
                <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Текущие склады</h4>
              ...
              <form onSubmit={handleAddWarehouse} className="pt-4 border-t border-slate-800 space-y-3">
                <h4 className="text-xs font-semibold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  Добавить новый склад филиалу (Централизованно)
                </h4>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="col-span-2">
                    <input
                      type="text"
                      required
                      placeholder="Название склада (e.g. Аптека / Склад №2)"
